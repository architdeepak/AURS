...
class drone_init:
    ...
    def zigzagsearch(self):
        ...
        def runinfer(stop_event):
            inference_start_time = time.time()
            k = 1
            print(f"inference for the {k} time")
            time.sleep(1) 
            while not stop_event.is_set():  
                print(f"sleeping for the {k} time")
                k += 1
                if self.run_inference():
                    self.detected = True
                    targetlat = vehicle.location.global_frame.lat
                    targetlon = vehicle.location.global_frame.lon
                ...
                print(f"Inference {k-1} took {inference_time:.2f} seconds") 

        inference_process = multiprocessing.Process(target=runinfer, args=(stop_event,))
        ...
        for i in range(num_passes):
            ...
            while not self.is_at_position(lat, lon_target, self.height):
                if self.detected:
                    break
                time.sleep(0.5)
            lon = lon_target
            print(f"Pass {i+1}/{num_passes} complete")
    ...
    def is_at_position(self, target_lat, target_lon, target_alt, tolerance=1):
        current = vehicle.location.global_relative_frame
        is_at = (
            abs(current.lat - target_lat) < tolerance / 111111 and
            abs(current.lon - target_lon) < tolerance / (111111 * math.cos(math.radians(current.lat))) and
            abs(current.alt - target_alt) < tolerance
        )
        return is_at
    ...
    def run_inference(self):
        ...
        filename = f"camerapic_{self.image_counter}.png"
        ...
        print(f"Model Response: {results}")  
        
        results_list = results.get('images', [{}])[0].get('results', [])
    ...
    def fly_to_target(self, input_lat, input_lon):
        print(f"Flying to target at {input_lat}, {input_lon}")
        self.move_to_fast(input_lat, input_lon, self.height)
        self.circle_target(input_lat, input_lon)
